...
Tester verification: RED REOPENED — 2026-08-11 at production commit `59ce4a9` with Node 24 (`PATH=/opt/homebrew/opt/node@24/bin:$PATH`). Focused unit/component command `npm test -- tests/component/WindowFrame.test.tsx tests/component/WindowLayer.test.tsx tests/component/PortfolioShell.test.tsx` previously passed 19/19; full `npm test` previously passed 11 files / 106 tests; `npm run test:coverage` previously passed all locked thresholds: statements 97.19% (346/356), branches 91.95% (263/286), functions 100% (79/79), and lines 97.76% (306/313). `npm run typecheck`, `npm run lint`, and `npm run build` previously passed. The new focused RED command `npm test -- tests/component/WindowLayer.test.tsx` fails only on `preserves desktop reducer positions through a coarse mobile measurement before returning wide`: expected About `{ x: 48, y: 72 }`, Work `{ x: 620, y: 96 }`, and Command `{ x: 360, y: 520 }`; actual after the coarse 390×400 measurement and wide return is About `{ x: 0, y: 10 }`, Work `{ x: 0, y: 0 }`, and Command `{ x: 0, y: 160 }`. This is the expected product RED, not a test-harness failure.
...
- Tester RED capture: `tests/component/WindowLayer.test.tsx` now exercises the fine/wide → coarse/narrow ResizeObserver measurement → fine/wide round trip and fails with the exact actual/expected positions above. `tests/e2e/window-system.spec.ts` adds the equivalent production-shell Work bounding-box round trip (1440×1000 → 390×844 → 1440×1000) so the Developer fix must preserve rendered desktop geometry as well as reducer state.
