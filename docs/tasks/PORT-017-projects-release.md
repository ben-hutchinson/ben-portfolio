...
Status: ACCEPTED — PORT-017 and the combined PORT-014–017 Portfolio OS polish release finalized at exact Node-24 Tester head `2f0d894cb2c0c4213e1543d7bcde468301d80f52` on 2026-08-20.
...
**Verdict:** ACCEPTED. PORT-017 and the combined PORT-014–017 Portfolio OS polish release are finalized at exact Node-24 Tester head `2f0d894cb2c0c4213e1543d7bcde468301d80f52` (`test: certify Node 24 portfolio release`). No production change after that head is part of this decision. This final decision supersedes the earlier exact-head acceptance at `e24d6f29be3a96238a6f76b91fbc3feb1a337724` and incorporates the subsequent final-review accessibility corrections.
...
- PORT-017: initial Tester RED `62ee3c43089e30f2f2539beb98e8216040191e6e`; project production `ed67e899d6459a7570ffddc68053ef9ac34b4b3d`; Career accessibility fix `533235c2bc34523bf125d2193321ccda26ad5c96`; prior release GREEN `e88fd4bceace68a3abd05c5ee0c50dfa652622fd`; rendered-order RED `2fa2c5b`; copy-first production fix `2c99bd60406b74eebc32f98cd2f4ac58e1acf991`; copy-first Tester GREEN `e24d6f29be3a96238a6f76b91fbc3feb1a337724`; prior PO acceptance `415c440e09e454f6853a4df195fd2bac9ce76cff`; final-review RED `5f31729`; focus-recovery production fix `dd29316bd072237afba8c07e69560e607eaa9fe5`; final Node-24 Tester GREEN `2f0d894cb2c0c4213e1543d7bcde468301d80f52`.

Exact-head release evidence is current and green under the locked runtime, Node **v24.19.0** and npm **11.17.0**: Career component **6/6 passed**; focused final-review Chromium **14 passed, 5 intentional skips, 0 failed**; focused Projects/responsive Chromium **8 passed, 1 intentional skip, 0 failed**; full E2E **50 passed, 160 intentional skips, 0 failed**; axe **2 passed, 8 intentional skips, 0 failed**; two consecutive no-update visual runs each passed **4/16/0**, with no baseline change. Coverage passed **160/160 tests** with statements **98.35% (539/548)**, branches **91.68% (386/421)**, functions **98.59% (140/142)**, and lines **99.37% (477/480)**, above the configured 91% thresholds. Clean install, typecheck, lint, static build, and `git diff --check` passed. Bundle evidence is **97,509 gzip JavaScript bytes / 204,800** and **118,529 initial image bytes / 1,048,576**.

The final Node-24 manual/static evidence passes `/ben-portfolio/`, direct hashes, browser history, keyboard-only navigation, visible Career range focus, reduced-motion zero-offset/zero-duration behavior, MicroTerminal Desktop-only behavior, link security, and Pokeleximon copy/media geometry. The four established visual baselines were manually inspected, passed the no-update gate twice, and remained unchanged. The final-review findings are closed: obsolete `#command` normalizes to `#desktop` and issues exactly one autofocus request to the labelled Portfolio command input, while fresh `#desktop`, `#unknown-route`, and `#projects/unknown-project` remain unfocused; the focusable Career stage viewport retains `tabindex="0"` and is now `role="region"` named by `aria-labelledby="career-timeline-heading"`.

**PORT-014 palette ruling:** the global `--color-ink` normalization from the earlier design-table value `#13221B` to approved Kernel Ink `#132218` is intentionally accepted as a site-wide palette alignment, not deferred scope or an accidental token change. It consistently aligns text, borders, controls, and hard shadows with the approved Kernel/favicon asset. The exact token and asset were covered by the release audit, and the full visual, axe, contrast, responsive, and regression gates validate its global consumers without requiring baseline changes at the final head.
...
**Worktree audit:** at final acceptance review, the only dirty entries were the protected pre-existing modified `.DS_Store` and untracked `.playwright-cli/`. Neither was modified or staged, and no other tracked or untracked change existed before these Product Owner records.
