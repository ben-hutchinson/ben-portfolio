...
Status: IN_TEST — Tester gate reopened for P1 skip-link routing defect
...
- [ ] Inspect the rendered shell against DESIGN §§3–5 and the locked visual direction. Reject a themed replica, decorative-only menu, hidden recruiter evidence, generic-card/dashboard treatment, or unreadably small production text.
- [ ] Inspect the first viewport at 1440×1000 in a ten-second recruiter scan: identity, role/trajectory, Manchester location, availability, primary navigation, Work/Career route, and exact flagship sentence are comprehensible without interaction.
- [ ] Inspect action convergence: menu, dock, and URL input all use the PORT-003 reducer/hash route, and CV/GitHub/LinkedIn are safe persistent actions under the GitHub Pages base path.
- [ ] Confirm no PORT-005/006 functionality has leaked into this packet.

## Acceptance criteria

- [ ] `tokens.css` is imported before global rules and provides the approved palette, typography, borders, hard shadows, compact radii, spacing, motion durations, and cross-surface focus treatment; component styles consume semantic tokens.
- [ ] The page presents a semantic, keyboard-operable shell with a working skip link, labelled primary navigation, `header`/`main`/`footer` landmarks, and a dock containing About, Work, Career, Projects, Contact, Command, and CV in stable order.
- [ ] The wide first viewport exposes Ben Hutchinson, platform-engineer role/trajectory, `Manchester, UK`, availability, primary navigation, and exactly `Migrated 50+ repositories and reduced average build time by four minutes.` without dragging, command input, or opening an application.
- [ ] Desktop/Work/Career/Projects route links, dock application actions, and active state are reducer/hash-backed; active menu route exposes `aria-current="page"`; project details correctly make Projects active; no UI component writes a hash or owns alternate navigation state.
- [ ] CV is a persistent base-safe download from the checked-in PDF, and GitHub/LinkedIn remain verified, labelled persistent external actions. None is an unsupported hash route or reducer action.
- [ ] Decorations are hidden from assistive technology, while a single polite live region announces only meaningful state changes and does not repeat static content or focus/hover noise.
- [ ] The output visibly follows the locked tactile mint/paper/Ink/yellow/orange language with local abstract glyphs and excludes glass, glow, space/command-centre visuals, commercial OS imitation, generic dashboard grids/cards, and tiny copy.
- [ ] At 1440×1000 and 390×844 the shell has no horizontal overflow; keyboard focus is conspicuous; compact mobile navigation/actions work in natural document order; and reduced motion leaves all required routes/actions usable.
- [ ] No drag, application interior, or recoverable window-system behaviour is introduced before PORT-005, and the interim recruiter summary remains bounded until PORT-006.
